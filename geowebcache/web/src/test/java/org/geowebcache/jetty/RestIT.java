...
public class RestIT {
    ...
    public void testCreateUpdateDelete() throws Exception {
        ...
        // Create
        {
            ...
            try( CloseableHttpResponse response = jetty.getClientAdmin().execute(request)) {
        ...
        // Delete
        {
            ...
            try( CloseableHttpResponse response = jetty.getClientAdmin().execute(request2)) {
        ...
        for(HttpUriRequest request: Arrays.asList(
            ...
            try( CloseableHttpResponse response = jetty.getClientAdmin().execute(request)) {
                ...
                assertThat(response.getStatusLine(), 
                        describedAs("Request %0 with authentication produces status code %1",
                                hasProperty("statusCode", equalTo(code)), 
                                request, code));
            }
            for(CloseableHttpClient client: Arrays.asList(
                    jetty.getClientAnonymous(),
                    jetty.getClientAdminBadPassword()
                    )) {
